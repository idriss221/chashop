...
function renderAllGrids() {
  const newArrivals = allProducts
    ...
    .concat(allProducts.filter((p) => p.badge !== "NOUVEAU"))
    .slice(0, 4);

  renderGrid("newArrivals", newArrivals);
  renderGrid(
    "shoesGrid",
    allProducts.filter((p) => p.type === "shoe"),
  );
  renderGrid(
    "clothesGrid",
    allProducts.filter((p) => p.type === "cloth"),
  );
  initCardAnimations();
}
...
document.body.addEventListener("click", (e) => {
  ...
  if (qtyUp) {
    changeQty(qtyUp.dataset.qtyUp, 1);
    return;
  }

  const qtyDown = e.target.closest("[data-qty-down]");
  if (qtyDown) {
    changeQty(qtyDown.dataset.qtyDown, -1);
    return;
  }
...
function findOrder(orderId, phone) {
  ...
  return (
    allOrders.find(
      (o) =>
        o.id.toUpperCase() === idNorm && normalizePhone(o.phone) === phoneNorm,
    ) || null
  );
}
